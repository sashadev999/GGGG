...
async def block_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # مرتب‌سازی کاربران بر اساس زمان عضویت
    sorted_users = []
    for user_id, user_data in users.items():
        join_date = user_data.get('join_date', '')
        sorted_users.append((user_id, user_data, join_date))
    
    # مرتب‌سازی بر اساس زمان عضویت (نزولی)
    sorted_users.sort(key=lambda x: x[2], reverse=True)
    
    buttons = []
    for user_id, user_data, _ in sorted_users:
        if not user_data.get('is_blocked', False):  # فقط کاربران غیر مسدود
            username = user_data.get('username', 'بدون یوزرنیم')
            first_name = user_data.get('first_name', 'بدون نام')
            join_date = datetime.fromisoformat(user_data.get('join_date', datetime.now().isoformat()))
            formatted_date = join_date.strftime("%Y/%m/%d %H:%M")
            buttons.append([InlineKeyboardButton(
                f"🚫 {first_name} (@{username}) - {formatted_date}", 
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(
        ...
        forward_to_admin
    ), group=0)  # گروه 0 برای اجرای قبل از سایر هندلرها
