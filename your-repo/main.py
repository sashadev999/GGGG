...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # اگر دکمه در گروه زده شده باشد
    if update.effective_chat.type in ['group', 'supergroup']:
        # فقط اجازه ارسال پیام همگانی به ادمین‌ها
        if is_admin(update.effective_user.id) and query.data == 'admin_broadcast':
            await handle_admin_broadcast(update, context)
        return  # برای سایر دکمه‌ها در گروه، هیچ عملی انجام نده
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # اگر پیام در گروه ارسال شده باشد
    if update.effective_chat.type in ['group', 'supergroup']:
        # فقط اجازه ارسال پیام همگانی به ادمین‌ها
        if is_admin(update.effective_user.id) and context.user_data.get('waiting_for_broadcast_message'):
            await handle_broadcast_message(update, context)
        return  # برای سایر پیام‌ها در گروه، هیچ عملی انجام نده
...
async def handle_buy_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'current_discount' in context.user_data:
        discount = context.user_data['current_discount']
        final_price = product['price'] - discount['amount']
    ...
    order = {
        ...
        'discount_code': context.user_data.get('current_discount', {}).get('code', None)
    }
...
💎 مبلغ تخفیف: {discount['amount']:,} تومان
"""
...
💳 6037-9977-0000-0000
بانک ملی - به نام: علی محمدی

⚠️ پس از پرداخت، تصویر رسید پرداخت را ارسال کنید.
"""
    ...
    # تنظیم وضعیت انتظار برای رسید پرداخت
    context.user_data['waiting_for_receipt'] = True
    context.user_data['current_order_id'] = order_id
    
    # پاک کردن اطلاعات تخفیف
    if 'current_discount' in context.user_data:
        del context.user_data['current_discount']
...
async def handle_discount_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if discount_code in db.data['discount_codes']:
        discount = db.data['discount_codes'][discount_code]
        ...
        # ذخیره اطلاعات تخفیف
        context.user_data['current_discount'] = {
            'code': discount_code,
            'percentage': discount['percentage'],
            'amount': discount_amount
        }
        
        # نمایش اطلاعات محصول با تخفیف
        product_text = f"""
{product['name']}
...
{product['description']}
        ...
        keyboard = [
            [InlineKeyboardButton("💳 خرید", callback_data=f'buy_{product_id}')],
            [InlineKeyboardButton("🎟 وارد کردن کد تخفیف دیگر", callback_data=f'discount_{product_id}')],
            [InlineKeyboardButton("🔙 بازگشت به لیست محصولات", callback_data='products')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(handle_buy_product, pattern='^buy_'))
    application.add_handler(CallbackQueryHandler(handle_discount_input, pattern='^discount_'))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_discount_code))
